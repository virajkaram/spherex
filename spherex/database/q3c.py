...
def create_postgis_index(db_name: str,
                        table_name: str,
                         column_name: str
                             ):
    """
    ...
    :return:
    """
    logger.debug(f"Checking if postgis spatial index exists on table {table_name}...")
    trig_ddl = DDL(f"SELECT indexname FROM pg_indexes WHERE tablename='{table_name}';")
    engine = get_engine(db_name=db_name)
    ...
    with engine.connect() as conn:
        ...
        if len(res) > 0:
            for index in res:
                if f"{table_name}_postgis_idx" in index:
                    logger.debug(f"postgis spatial index already exists on table {table_name}")
                    return

    logger.info(f"Creating postgis extension on database {db_name}...")
    trig_ddl = DDL(
        f"CREATE INDEX IF NOT EXISTS "
        f"{table_name}_postgis_idx ON {table_name} "
        f"USING GIST ({column_name});"
    )
    engine = get_engine(db_name=db_name)
